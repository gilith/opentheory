...
let BETA_CONV = log_function "BETA_CONV" log_term log_thm BETA_CONV;;
...
let AP_TERM = log_function2 "AP_TERM" log_term log_thm log_thm AP_TERM;;

let AP_THM th tm =
  try MK_COMB(th,REFL tm)
  with Failure _ -> failwith "AP_THM";;

let AP_THM = log_function2 "AP_THM" log_thm log_term log_thm AP_THM;;
...
let SYM = log_function "SYM" log_thm log_thm SYM;;

let ALPHA tm1 tm2 =
  try TRANS (REFL tm1) (REFL tm2)
  with Failure _ -> failwith "ALPHA";;

let ALPHA = log_function2 "ALPHA" log_term log_term log_thm ALPHA;;
...
let ALPHA_CONV =
    log_function2 "ALPHA_CONV" log_term log_term log_thm ALPHA_CONV;;
...
let GEN_ALPHA_CONV =
    log_function2 "GEN_ALPHA_CONV" log_term log_term log_thm GEN_ALPHA_CONV;;

let MK_BINOP op (lth,rth) =
  MK_COMB(AP_TERM op lth,rth);;

let MK_BINOP =
    log_function2 "MK_BINOP" log_term (log_pair log_thm log_thm)
      log_thm MK_BINOP;;
...
let NO_CONV = log_function "NO_CONV" log_term log_thm NO_CONV;;

let (ALL_CONV:conv) = REFL;;

let ALL_CONV = log_function "ALL_CONV" log_term log_thm ALL_CONV;;
...
let (CHANGED_CONV:conv->conv) =
  fun conv tm ->
    log_lemma "CHANGED_CONV _ _" (fun () ->
    let th = conv tm in
...
let (COMB2_CONV: conv->conv->conv) =
  fun lconv rconv tm ->
    log_lemma "COMB2_CONV _ _" (fun () ->
    let l,r = dest_comb tm in MK_COMB(lconv l,rconv r));;

let COMB_CONV = W COMB2_CONV;;

let (ABS_CONV:conv->conv) =
  fun conv tm ->
    log_lemma "ABS_CONV _ _" (fun () ->
...
let (ONCE_DEPTH_CONV: conv->conv),
  ...
  and THENCQC conv1 conv2 tm =
    log_lemma "THENCQC _ _ _" (fun () ->
    let th1 = conv1 tm in
    try let th2 = conv2(rand(concl th1)) in TRANS th1 th2
    with Failure _ -> th1)
  and COMB_QCONV conv tm =
    log_lemma "COMB_QCONV _ _" (fun () ->
...
let rec DEPTH_BINOP_CONV op conv tm =
  log_lemma "DEPTH_BINOP_CONV _ _ _" (fun () ->
  match tm with
...
let SYM_CONV = log_function "SYM_CONV" log_term log_thm SYM_CONV;;
...
let SUBS_CONV =
    log_function2 "SUBS_CONV" (log_list log_thm) log_term log_thm SUBS_CONV;;
...
let BETA_RULE = log_function "BETA_RULE" log_thm log_thm BETA_RULE;;

let GSYM = CONV_RULE(ONCE_DEPTH_CONV SYM_CONV);;

let GSYM = log_function "GSYM" log_thm log_thm GSYM;;

let SUBS ths = CONV_RULE (SUBS_CONV ths);;

let SUBS = log_function2 "SUBS" (log_list log_thm) log_thm log_thm SUBS;;
...
let CACHE_CONV =
  let ALPHA_HACK th =
    ...
    fun tm ->
      log_lemma "CACHE_CONV.ALPHA_HACK _ _" (fun () ->
