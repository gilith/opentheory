...
let REAL_ADD_AC = log_lemma "REAL_ADD_AC" (fun () -> prove
 (`(m + n = n + m) /\
   ((m + n) + p = m + (n + p)) /\
   (m + (n + p) = n + (m + p))`,
  MESON_TAC[REAL_ADD_ASSOC; REAL_ADD_SYM]));;

let REAL_ADD_RINV = log_lemma "REAL_ADD_RINV" (fun () -> prove
 (`!x. x + --x = &0`,
  MESON_TAC[REAL_ADD_SYM; REAL_ADD_LINV]));;

let REAL_EQ_ADD_LCANCEL = log_lemma "REAL_EQ_ADD_LCANCEL" (fun () -> prove
 (`!x y z. (x + y = x + z) <=> (y = z)`,
...
let REAL_EQ_ADD_RCANCEL = log_lemma "REAL_EQ_ADD_RCANCEL" (fun () -> prove
 (`!x y z. (x + z = y + z) <=> (x = y)`,
  MESON_TAC[REAL_ADD_SYM; REAL_EQ_ADD_LCANCEL]));;

let REAL_MUL_RZERO = log_lemma "REAL_MUL_RZERO" (fun () -> prove
 (`!x. x * &0 = &0`,
  MESON_TAC[REAL_EQ_ADD_RCANCEL; REAL_ADD_LDISTRIB; REAL_ADD_LID]));;

let REAL_MUL_LZERO = log_lemma "REAL_MUL_LZERO" (fun () -> prove
 (`!x. &0 * x = &0`,
  MESON_TAC[REAL_MUL_SYM; REAL_MUL_RZERO]));;

let REAL_NEG_NEG = log_lemma "REAL_NEG_NEG" (fun () -> prove
 (`!x. --(--x) = x`,
  MESON_TAC
   [REAL_EQ_ADD_RCANCEL; REAL_ADD_LINV; REAL_ADD_SYM; REAL_ADD_LINV]));;

let REAL_MUL_RNEG = log_lemma "REAL_MUL_RNEG" (fun () -> prove
 (`!x y. x * (--y) = -- (x * y)`,
  MESON_TAC[REAL_EQ_ADD_RCANCEL; REAL_ADD_LDISTRIB; REAL_ADD_LINV;
            REAL_MUL_RZERO]));;

let REAL_MUL_LNEG = log_lemma "REAL_MUL_LNEG" (fun () -> prove
 (`!x y. (--x) * y = -- (x * y)`,
  MESON_TAC[REAL_MUL_SYM; REAL_MUL_RNEG]));;

let REAL_NEG_ADD = log_lemma "REAL_NEG_ADD" (fun () -> prove
 (`!x y. --(x + y) = --x + --y`,
  REPEAT GEN_TAC THEN
...
let REAL_ADD_RID = log_lemma "REAL_ADD_RID" (fun () -> prove
 (`!x. x + &0 = x`,
  MESON_TAC[REAL_ADD_SYM; REAL_ADD_LID]));;

let REAL_NEG_0 = log_lemma "REAL_NEG_0" (fun () -> prove
 (`--(&0) = &0`,
  MESON_TAC[REAL_ADD_LINV; REAL_ADD_RID]));;

let REAL_LE_LNEG = log_lemma "REAL_LE_LNEG" (fun () -> prove
 (`!x y. --x <= y <=> &0 <= x + y`,
...
let REAL_LE_NEG2 = log_lemma "REAL_LE_NEG2" (fun () -> prove
 (`!x y. --x <= --y <=> y <= x`,
  REPEAT GEN_TAC THEN
...
let REAL_LE_RNEG = log_lemma "REAL_LE_RNEG" (fun () -> prove
 (`!x y. x <= --y <=> x + y <= &0`,
  REPEAT GEN_TAC THEN
...
let REAL_OF_NUM_POW = log_lemma "REAL_OF_NUM_POW" (fun () -> prove
 (`!x n. (&x) pow n = &(x EXP n)`,
  GEN_TAC THEN INDUCT_TAC THEN
  ASM_REWRITE_TAC[real_pow; EXP; REAL_OF_NUM_MUL]));;

let REAL_POW_NEG = log_lemma "REAL_POW_NEG" (fun () -> prove
...
let REAL_ABS_NUM = log_lemma "REAL_ABS_NUM" (fun () -> prove
 (`!n. abs(&n) = &n`,
  REWRITE_TAC[real_abs; REAL_OF_NUM_LE; LE_0]));;

let REAL_ABS_NEG = log_lemma "REAL_ABS_NEG" (fun () -> prove
 (`!x. abs(--x) = abs x`,
...
let REAL_INT_LE_CONV,REAL_INT_LT_CONV,
  ...
  let [pth_le1; pth_le2a; pth_le2b; pth_le3] = log_lemmas "REAL_INT_LE_CONV.[pth_le1; pth_le2a; pth_le2b; pth_le3]" (fun () -> (CONJUNCTS o prove)
   (`(--(&m) <= &n <=> T) /\
     (&m <= &n <=> m <= n) /\
     (--(&m) <= --(&n) <=> n <= m) /\
     (&m <= --(&n) <=> (m = 0) /\ (n = 0))`,
  ...
  let [pth_lt1; pth_lt2a; pth_lt2b; pth_lt3] = log_lemmas "REAL_INT_LE_CONV.[pth_lt1; pth_lt2a; pth_lt2b; pth_lt3]" (fun () -> (CONJUNCTS o prove)
   (`(&m < --(&n) <=> F) /\
     (&m < &n <=> m < n) /\
     (--(&m) < --(&n) <=> n < m) /\
     (--(&m) < &n <=> ~((m = 0) /\ (n = 0)))`,
  ...
  let [pth_ge1; pth_ge2a; pth_ge2b; pth_ge3] = log_lemmas "REAL_INT_LE_CONV.[pth_ge1; pth_ge2a; pth_ge2b; pth_ge3]" (fun () -> (CONJUNCTS o prove)
   (`(&m >= --(&n) <=> T) /\
     (&m >= &n <=> n <= m) /\
     (--(&m) >= --(&n) <=> m <= n) /\
     (--(&m) >= &n <=> (m = 0) /\ (n = 0))`,
  ...
  let [pth_gt1; pth_gt2a; pth_gt2b; pth_gt3] = log_lemmas "REAL_INT_LE_CONV.[pth_gt1; pth_gt2a; pth_gt2b; pth_gt3]" (fun () -> (CONJUNCTS o prove)
   (`(--(&m) > &n <=> F) /\
     (&m > &n <=> n < m) /\
     (--(&m) > --(&n) <=> m < n) /\
     (&m > --(&n) <=> ~((m = 0) /\ (n = 0)))`,
  ...
  let [pth_eq1a; pth_eq1b; pth_eq2a; pth_eq2b] = log_lemmas "REAL_INT_LE_CONV.[pth_eq1a; pth_eq1b; pth_eq2a; pth_eq2b]" (fun () -> (CONJUNCTS o prove)
   (`((&m = &n) <=> (m = n)) /\
     ((--(&m) = --(&n)) <=> (m = n)) /\
     ((--(&m) = &n) <=> (m = 0) /\ (n = 0)) /\
     ((&m = --(&n)) <=> (m = 0) /\ (n = 0))`,
...
let REAL_INT_NEG_CONV =
  let pth = log_lemma "REAL_INT_NEG_CONV.pth" (fun () -> prove
   (`(--(&0) = &0) /\
     (--(--(&x)) = &x)`,
...
let REAL_INT_MUL_CONV =
  let pth0 = log_lemma "REAL_INT_MUL_CONV.pth0" (fun () -> prove
   (`(&0 * &x = &0) /\
     (&0 * --(&x) = &0) /\
     (&x * &0 = &0) /\
     (--(&x) * &0 = &0)`,
    REWRITE_TAC[REAL_MUL_LZERO; REAL_MUL_RZERO]))
  and pth1,pth2 = log_lemma2 "REAL_INT_MUL_CONV.pth1,2" (fun () -> (CONJ_PAIR o prove)
   (`((&m * &n = &(m * n)) /\
      (--(&m) * --(&n) = &(m * n))) /\
     ((--(&m) * &n = --(&(m * n))) /\
      (&m * --(&n) = --(&(m * n))))`,
...
let REAL_INT_ADD_CONV =
  ...
  let pth0 = log_lemma "REAL_INT_ADD_CONV.pth0" (fun () -> prove
   (`(--(&m) + &m = &0) /\
     (&m + --(&m) = &0)`,
    REWRITE_TAC[REAL_ADD_LINV; REAL_ADD_RINV])) in
  let [pth1; pth2; pth3; pth4; pth5; pth6] = log_lemmas "REAL_INT_ADD_CONV.[pth1; pth2; pth3; pth4; pth5; pth6]" (fun () -> (CONJUNCTS o prove)
   (`(--(&m) + --(&n) = --(&(m + n))) /\
     (--(&m) + &(m + n) = &n) /\
     (--(&(m + n)) + &m = --(&n)) /\
     (&(m + n) + --(&m) = &n) /\
...
let REAL_INT_POW_CONV =
  let pth1,pth2 = log_lemma2 "REAL_INT_POW_CONV.pth1,pth2" (fun () -> (CONJ_PAIR o prove)
   (`(&x pow n = &(x EXP n)) /\
     ((--(&x)) pow n = if EVEN n then &(x EXP n) else --(&(x EXP n)))`,
    REWRITE_TAC[REAL_OF_NUM_POW; REAL_POW_NEG])) in
  let tth = log_lemma "REAL_INT_POW_CONV.tth" (fun () -> prove
...
let REAL_INT_ABS_CONV =
  let pth = log_lemma "REAL_INT_ABS_CONV.pth" (fun () -> prove
   (`(abs(--(&x)) = &x) /\
     (abs(&x) = &x)`,
