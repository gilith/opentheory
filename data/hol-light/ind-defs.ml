...
let RIGHT_BETAS =
  log_function2 "RIGHT_BETAS" (log_list log_term) log_thm log_thm RIGHT_BETAS;;
...
let EXISTS_EQUATION =
  let pth = log_lemma "EXISTS_EQUATION.pth" (fun () -> prove
   (`!P t. (!x:A. (x = t) ==> P x) ==> (?) P`,
    ...
    REFL_TAC)) in
  fun tm th ->
    log_lemma "EXISTS_EQUATION _ _" (fun () ->
...
let derive_nonschematic_inductive_relations =
  log_function "derive_nonschematic_inductive_relations" log_term log_thm
...
let MONO_FORALL = log_lemma "MONO_FORALL" (fun () -> prove
 (`(!x:A. P x ==> Q x) ==> ((!x. P x) ==> (!x. Q x))`,
  REPEAT STRIP_TAC THEN FIRST_ASSUM MATCH_MP_TAC THEN
  ASM_REWRITE_TAC[]));;

let MONO_EXISTS = log_lemma "MONO_EXISTS" (fun () -> prove
 (`(!x:A. P x ==> Q x) ==> ((?x. P x) ==> (?x. Q x))`,
...
let prove_monotonicity_hyps =
  ...
  fun th ->
    log_lemma "prove_monotonicity_hyps _" (fun () ->
...
let prove_inductive_relations_exist =
  log_function "prove_inductive_relations_exist" log_term log_thm
    prove_inductive_relations_exist;;

let new_inductive_definition =
  log_function "new_inductive_definition" log_term
    (log_triple log_thm log_thm log_thm) new_inductive_definition;;
...
let derive_strong_induction =
  log_function "derive_strong_induction" (log_pair log_thm log_thm)
    log_thm derive_strong_induction;;
