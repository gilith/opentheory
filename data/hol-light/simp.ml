...
let REWR_CONV = log_function2 "REWR_CONV" log_thm log_term log_thm REWR_CONV;;

let IMP_REWR_CONV = PART_MATCH (lhs o snd o dest_imp);;

let IMP_REWR_CONV =
    log_function2 "IMP_REWR_CONV" log_thm log_term log_thm IMP_REWR_CONV;;
...
let ORDERED_REWR_CONV ord th =
  ...
  fun tm ->
    log_lemma "ORDERED_REWR_CONV _ _ _" (fun () ->
...
let ORDERED_IMP_REWR_CONV ord th =
  ...
  fun tm ->
    log_lemma "ORDERED_IMP_REWR_CONV _ _ _" (fun () ->
    let thm = basic_conv tm in
...
let mk_rewrites =
  let IMP_CONJ_CONV = REWR_CONV(log_lemma "mk_rewrites.IMP_CONJ_CONV" (fun () ->
                                ITAUT `p ==> q ==> r <=> p /\ q ==> r`))
  and IMP_EXISTS_RULE =
    let cnv = REWR_CONV(log_lemma "mk_rewrites.IMP_EXISTS_RULE" (fun () ->
                        ITAUT `(!x. P x ==> Q) <=> (?x. P x) ==> Q`)) in
    fun v th -> CONV_RULE cnv (GEN v th) in
  let collect_condition oldhyps th =
    log_lemma "mk_rewrites.collect_condition _ _" (fun () ->
...
let basic_prover strat (Simpset(net,prover,provers,rewmaker) as ss) lev tm =
  log_lemma "basic_prover _ _ _ _" (fun () ->
...
let set_basic_rewrites,extend_basic_rewrites,basic_rewrites,
  ...
  and extend_basic_rewrites thl =
    let canon_thl = log_value "extend_basic_rewrites _" (log_list log_thm)
                    (fun () -> itlist (mk_rewrites false) thl []) in
...
let SIMP_CONV =
  log_function2 "SIMP_CONV" (log_list log_thm) log_term log_thm SIMP_CONV;;

let PURE_SIMP_CONV thl = SIMPLIFY_CONV empty_ss thl;;

let PURE_SIMP_CONV =
  log_function2 "PURE_SIMP_CONV" (log_list log_thm) log_term log_thm
    PURE_SIMP_CONV;;

let ONCE_SIMP_CONV thl = ONCE_SIMPLIFY_CONV (basic_ss []) thl;;

let ONCE_SIMP_CONV =
  log_function2 "ONCE_SIMP_CONV" (log_list log_thm) log_term log_thm
    ONCE_SIMP_CONV;;
