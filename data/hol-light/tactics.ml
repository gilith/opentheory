...
let _FALSITY_ =
  log_lemma "_FALSITY_" (fun () -> new_definition `_FALSITY_ = F`);;

let mk_fthm =
  let pth = log_lemma "mk_fthm.pth" (fun () ->
            UNDISCH(fst(EQ_IMP_RULE _FALSITY_)))
  and qth = log_lemma "mk_fthm.qth" (fun () ->
            ASSUME `_FALSITY_`) in
  fun (asl,c) -> log_lemma "mk_fthm _" (fun () ->
                 PROVE_HYP qth (itlist ADD_ASSUM (rev asl) (CONTR c pth)));;
...
let (VALID:tactic->tactic) =
  let fake_thm (asl,w) =
    log_lemma "VALID.fake_thm _" (fun () ->
...
let (THEN),(THENL) =
  ...
  let justsequence just1 just2 insts2 i ths =
    log_lemma "THEN/THENL.justsequence _ _ _ _ _" (fun () ->
...
let (LABEL_TAC: string -> thm_tactic) =
  fun s thm (asl,w) ->
    ...
    fun i [th] ->
    log_lemma "LABEL_TAC _ _ _ _ _" (fun () ->
    PROVE_HYP (INSTANTIATE_ALL i thm) th);;
...
let (CONV_TAC: conv -> tactic) =
  ...
  fun conv ((asl,w) as g) ->
    ...
    let th' = SYM th in
    null_meta,[asl,r],fun i [th] -> log_lemma "#3 (CONV_TAC _ _) _ _" (fun () ->
                                    EQ_MP (INSTANTIATE_ALL i th') th);;
...
let (ABS_TAC: tactic) =
  fun (asl,w) ->
    try let l,r = dest_eq w in
        ...
        fun i [th] -> log_lemma "(ABS_TAC _).3 _ _" (fun () ->
                      let ath = ABS v th in
...
let (DISCH_TAC: tactic) =
  ...
  fun (asl,w) ->
    ...
    with Failure _ -> try
        ...
        fun i [th] -> log_lemma "(DISCH_TAC _).3 _ _" (fun () ->
                      NOT_INTRO(DISCH (instantiate i ant) th))
    with Failure _ -> failwith "DISCH_TAC";;

let (MP_TAC: thm_tactic) =
  fun thm (asl,w) ->
    null_meta,[asl,mk_imp(concl thm,w)],
    fun i [th] -> log_lemma "(MP_TAC _ _).3 _ _" (fun () ->
                  MP th (INSTANTIATE_ALL i thm));;
...
let (UNDISCH_TAC: term -> tactic) =
 fun tm (asl,w) ->
   try let sthm,asl' = remove (fun (_,asm) -> aconv (concl asm) tm) asl in
       ...
       fun i [th] -> log_lemma "(UNDISCH_TAC _ _).3 _ _" (fun () ->
                     MP th (INSTANTIATE_ALL i thm))
  ...
  fun x' ->
    ...
    fun (asl,w) ->
      try let x,bod = dest_forall w in
          ...
          fun i [th] -> log_lemma "(X_GEN_TAC _ _).3 _ _" (fun () ->
                        afn (GEN x' th))
  ...
  fun x' xth ->
    try let xtm = concl xth in
        ...
        fun (asl,w) ->
          ...
          null_meta,[("",xth')::asl,w],
          fun i [th] -> log_lemma "(X_CHOOSE_TAC _ _ _).3 _ _" (fun () ->
                        CHOOSE(x',INSTANTIATE_ALL i xth) th)
  ...
  fun dth ->
    try let dtm = concl dth in
        ...
        fun (asl,w) ->
          ...
          fun i [th1;th2] -> log_lemma "(DISJ_CASES_TAC _ _).3 _ _" (fun () ->
                             DISJ_CASES (INSTANTIATE_ALL i dth) th1 th2)
...
let (MATCH_MP_TAC :thm_tactic) =
  fun th ->
    let sth =
      log_lemma "(MATCH_MP_TAC _).sth" (fun () ->
    ...
    fun (asl,w) -> try let xth = match_fun w in
                       ...
                       null_meta,[asl,lant],
                       fun i [th] -> log_lemma "(MATCH_MP_TAC _ _).3 _ _"
                                     (fun () -> MP (INSTANTIATE_ALL i xth) th)
...
let (CONJUNCTS_THEN2:thm_tactic->thm_tactic->thm_tactic) =
  fun ttac1 ttac2 cth ->
      ...
      fun gl -> let ti,gls,jfn = (ttac1(ASSUME c1) THEN ttac2(ASSUME c2)) gl in
                let jfn' i ths =
                  log_lemma "(CONJUNCTS_THEN2 _ _ _ _).jfn _ _" (fun () ->
...
let (SUBGOAL_THEN: term -> thm_tactic -> tactic) =
  fun wa ttac (asl,w) ->
    ...
    meta,(asl,wa)::gl,fun i l -> log_lemma "(SUBGOAL_THEN _ _ _).3 _ _"
                                 (fun () -> PROVE_HYP (hd l) (just i (tl l)));;
...
let (META_SPEC_TAC: term -> thm -> tactic) =
  fun t thm (asl,w) ->
    ...
    fun i [th] -> log_lemma "(META_SPEC_TAC _ _ _).3 _ _" (fun () ->
                  PROVE_HYP (SPEC (instantiate i t) thm) th);;
...
let ANTS_TAC =
  ...
  and tm2 = `p ==> q` in
  let th =
    log_lemma "ANTS_TAC.th" (fun () ->
    let th1,th2 = CONJ_PAIR(ASSUME tm1) in
    itlist DISCH [tm1;tm2] (MP th2 (MP(ASSUME tm2) th1))) in
