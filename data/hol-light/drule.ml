...
let log_instantiation : instantiation -> unit =
    log_triple (log_list (log_pair log_num log_term)) log_inst log_type_inst;;
...
let mk_thm =
  log_function "mk_thm" (log_pair (log_list log_term) log_term) log_thm mk_thm;;
...
let MK_CONJ =
  log_function2 "MK_CONJ" log_thm log_thm log_thm MK_CONJ;;
...
let MK_DISJ =
  log_function2 "MK_DISJ" log_thm log_thm log_thm MK_DISJ;;
...
let MK_FORALL =
  log_function2 "MK_FORALL" log_term log_thm log_thm MK_FORALL;;
...
let MK_EXISTS =
  log_function2 "MK_EXISTS" log_term log_thm log_thm MK_EXISTS;;
...
let BETAS_CONV =
  log_function "BETAS_CONV" log_term log_thm BETAS_CONV;;
...
let INSTANTIATE =
  log_function2 "INSTANTIATE" log_instantiation log_thm log_thm INSTANTIATE;;
...
let INSTANTIATE_ALL =
  log_function2 "INSTANTIATE_ALL" log_instantiation log_thm log_thm
    INSTANTIATE_ALL;;
...
let PART_MATCH,GEN_PART_MATCH =
  ...
  and GEN_PART_MATCH partfn th =
    let sth = log_lemma "GEN_PART_MATCH _ _.sth" (fun () -> SPEC_ALL th) in
    let bod = concl sth in
    let pbod = partfn bod in
    let lconsts = intersect (frees (concl th)) (freesl(hyp th)) in
    let fvs = subtract (subtract (frees bod) (frees pbod)) lconsts in
    fun tm ->
      log_lemma "GEN_PART_MATCH _ _ _" (fun () ->
      let bvms = match_bvs tm pbod [] in
      let abod = deep_alpha bvms bod in
      let ath = EQ_MP (ALPHA bod abod) sth in
      let insts = term_match lconsts (partfn abod) tm in
...
let MATCH_MP =
  log_function2 "MATCH_MP" log_thm log_thm log_thm MATCH_MP;;
...
let HIGHER_REWRITE_CONV =
  log_function3 "HIGHER_REWRITE_CONV" (log_list log_thm) log_bool log_term
    log_thm HIGHER_REWRITE_CONV;;
