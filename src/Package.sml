...
local
  ...
  val packageConstraintParser =
      (packageKeywordParser ++ atLeastOneSpace ++
       packageNameParser ++ manySpace ++ terminatorParser) >>
      (fn ((),((),(p,((),())))) => PackageConstraint p);

  val interpretConstraintParser =
      (interpretKeywordParser ++ atLeastOneSpace ++
       Interpretation.parserRewrite ++ manySpace ++ terminatorParser) >>
      (fn ((),((),(r,((),())))) => InterpretConstraint r);

  val importConstraintParser =
      (importKeywordParser ++ atLeastOneSpace ++
       requireNameParser ++ manySpace ++ terminatorParser) >>
